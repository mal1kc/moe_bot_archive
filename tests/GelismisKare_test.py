...
from moe_bot.temel_siniflar import GelismisKare, Kare


@pytest.fixture
def ornek_kare() -> GelismisKare:
    return GelismisKare(150, 295, 104, 72)
...
def test_kare_init(ornek_kare):
    ...
    assert GelismisKare(Kare(150, 295, 104, 72)), "GelismisKare init kare hatası"
    assert GelismisKare(kaynak_kare.x, kaynak_kare.y, kaynak_kare.genislik, kaynak_kare.yukseklik), "GelismisKare init kordinat hatası"


def test_kare_str(ornek_kare):
    kaynak_kare = ornek_kare
    assert (
        str(kaynak_kare) == f"GelismisKare({kaynak_kare.x},{kaynak_kare.y},{kaynak_kare.genislik},{kaynak_kare.yukseklik})"
    ), f"GelismisKare str hatası\n beklenen: GelismisKare({kaynak_kare.x},{kaynak_kare.y},{kaynak_kare.genislik},{kaynak_kare.yukseklik})\n alınan: {kaynak_kare}"  # noqa: E501


def test_kare_repr(ornek_kare):
    kaynak_kare = ornek_kare
    assert (
        repr(kaynak_kare) == f"GelismisKare({kaynak_kare.x},{kaynak_kare.y},{kaynak_kare.genislik},{kaynak_kare.yukseklik})"
    ), f"GelismisKare repr hatası\n beklenen: GelismisKare({kaynak_kare.x},{kaynak_kare.y},{kaynak_kare.genislik},{kaynak_kare.yukseklik})\n alınan: {kaynak_kare}"  # noqa: E501
...
def test_kare_hash2(ornek_kare):
    ...
    assert set([kaynak_kare]) == set(
        [GelismisKare(150, 295, 104, 72), kaynak_kare]
    ), f"GelismisKare hash hatası\n beklenen: {set([kaynak_kare])}\n alınan: {set([kaynak_kare])}"
...
def kare_olustur_icindeki_kare(ornek_kare, buyume_miktari):
    ...
    return GelismisKare(x, y, genislik, yukseklik)
